...
(() => {
  ...
  let period='day',tab='dashboard',editType='',editId=null;
  ...
  function openEditor(type,id=null,selected=null){editType=type;editId=id;const obj=(type==='stock'?state.stocks:state.savings).find(x=>x.id===id)||selected||{};$('#editor-title').textContent=`${id?'수정':'추가'} · ${type==='stock'?'투자 자산':'은행 자산'}`;
    if(type==='stock'){$('#editor-fields').innerHTML=`<div style="grid-column:1/-1"><button type="button" class="btn primary" id="open-market-search">종목/티커 검색</button></div>`+field('name','종목명',obj.name,'text','required')+field('ticker','티커 (선택)',obj.ticker)+field('market','시장 (검색으로 자동 입력)',obj.market||'')+field('buy','매수 단가',obj.buy??'','number','min="0" step="any" required')+field('quantity','보유 수량',obj.quantity??'','number','min="0" step="any" required')+field('price','현재가 (선택, 빈칸은 평단 적용)',obj.price,'number','min="0" step="any"')+`<label class="field">통화<select name="foreign"><option value="false" ${!obj.foreign?'selected':''}>KRW (국내·가상화폐)</option><option value="true" ${obj.foreign?'selected':''}>USD (해외 주식)</option></select></label>`+field('buyFx','매수 당시 USD/KRW 환율 (선택)',obj.buyFx,'number','min="1" step="any"')+`<label class="field">리스크 분류<select name="risk">${[...new Set([...state.config.risks,...(obj.risk?[obj.risk]:[])])].map(r=>`<option value="${safe(r)}" ${obj.risk===r?'selected':''}>${safe(r)}</option>`).join('')}</select></label>`}
  ...
  $('#editor-form').addEventListener('submit',e=>{e.preventDefault();const f=new FormData(e.currentTarget),val=k=>String(f.get(k)||'').trim(),n=k=>Number(f.get(k));let item;if(editType==='stock'){item={id:editId||uid(),name:val('name'),ticker:val('ticker').toUpperCase(),buy:n('buy'),quantity:n('quantity'),price:val('price')===''?null:n('price'),foreign:val('foreign')==='true',risk:val('risk'),market:val('market')||(val('foreign')==='true'?'US':'KRX'),buyFx:val('buyFx')===''?null:n('buyFx')};if(item.foreign&&item.buyFx!==null&&item.buyFx<=0){toast('매수 환율은 0보다 커야 합니다.');return}}
    ...
    if(editType==='stock'){
      ...
      const old=arr[idx];if(old&&old.price===item.price&&old.ticker===item.ticker&&old.foreign===item.foreign)Object.assign(item,{quoteAsOf:old.quoteAsOf,quoteSource:old.quoteSource,quoteError:old.quoteError,quoteKind:old.quoteKind});else Object.assign(item,{quoteAsOf:new Date().toISOString(),quoteSource:'수동 입력',quoteError:''});
    ...
    try{state=normalize(next)}catch(err){toast(err.message);return}$('#editor').close();if(persist())toast('자산이 저장되었습니다.')});
  document.addEventListener('click',e=>{const t=e.target.closest('button');if(!t)return;if(t.dataset.close!==undefined){t.closest('dialog').close();return}if(t.dataset.tab){tab=t.dataset.tab;document.querySelectorAll('.tab').forEach(x=>x.classList.toggle('active',x.dataset.tab===tab));document.querySelectorAll('.panel').forEach(x=>x.classList.toggle('active',x.id===tab));if(tab==='dashboard')renderHistory();return}if(t.dataset.period){period=t.dataset.period;document.querySelectorAll('[data-period]').forEach(x=>x.classList.toggle('selected',x.dataset.period===period));renderHistory();return}if(t.id==='open-market-search'){openMarketSearch();return}if(t.dataset.marketPick!==undefined){const x=$('#market-results')._items?.[Number(t.dataset.marketPick)];if(x){const f=$('#editor-form');f.elements.name.value=x.name||x.symbol;f.elements.ticker.value=x.ticker||x.symbol;f.elements.market.value=x.market;f.elements.foreign.value=x.currency==='USD'?'true':'false';$('#market-search').close();toast('종목을 선택했습니다.')}return}if(t.dataset.edit){openEditor(t.dataset.edit,t.dataset.id);return}if(t.dataset.delete){const arr=t.dataset.delete==='stock'?state.stocks:state.savings;const entry=arr.find(x=>x.id===t.dataset.id);if(entry&&confirm(`${entry.name} 자산을 삭제할까요?`)){arr.splice(arr.indexOf(entry),1);delete state.allocations[entry.id];if(persist())toast('삭제했습니다.')}return}});
