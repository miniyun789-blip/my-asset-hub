        ...
        if match:
            ...
            if st.sidebar.button(btn_label, use_container_width=True):
                if existing_idx is not None:
                    ...
                    st.session_state['stocks'].append({"종목명": sel_name, "티커": sel_code, "매수평단가": new_price, "보유수량": new_qty, "해외여부": is_foreign, "리스크": risk_level.split(" ")[0]})
                sort_assets()
                st.rerun()
    else:
