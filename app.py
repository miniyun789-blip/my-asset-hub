...
# ==========================================
st.set_page_config(page_title="My Asset Hub V19", layout="wide")
    ...
    if valid_ticker:
        ...
        if st.sidebar.button(btn_label, use_container_width=True):
            if existing_idx is not None:
                ...
                final_qty = old['보유수량'] + new_qty
                final_price = ((old['매수평단가'] * old['보유수량']) + (new_price * new_qty)) / final_qty if final_qty > 0 else 0
...
for idx, stock in enumerate(st.session_state['stocks']):
    ...
    if is_crypto:
        ...
        buy_amt = stock['매수평단가'] * stock['보유수량']
    else:
        curr, _ = get_stock_data(ticker)
        curr = curr * exchange_rate if is_foreign else curr
        buy_amt = stock['매수평단가'] * stock['보유수량'] * (exchange_rate if is_foreign else 1)
    
    eval_amt = curr * stock['보유수량']
    total_buy += buy_amt
    ...
    stock_display.append({"ID": idx, "종목명": stock['종목명'], "티커": ticker, "매수": buy_amt, "평가": eval_amt, "수익률": (eval_amt-buy_amt)/buy_amt*100 if buy_amt>0 else 0, "리스크": risk_cat, "현재가": curr, "해외": is_foreign})

total_sav_val = 0
fixed_sav_val = 0
for sav in st.session_state['savings']:
    amt = sav['월납입액'] * sav['현재회차']
    total_sav_val += amt
    total_buy += amt
    if sav['종류'] in ["적금", "주택청약"]: fixed_sav_val += amt
...
with tab1:
    ...
    if not history_df.empty:
        ...
        # [수정] 모바일 가독성 극대화를 위한 차트 옵션 적용
        if time_view == "일별":
            fig_line = px.area(plot_df, x="날짜", y="총자산", markers=True, color_discrete_sequence=['#2E86C1'])
            fig_line.update_xaxes(tickformat="%m.%d", tickangle=-45) # 좁은 모바일 화면을 위해 기울임
        elif time_view == "월별":
        ...
        fig_line.update_layout(
            ...
            height=300,
            hovermode="x unified" # 모바일 터치 시 선명하게 정보 표시
        )
...
with tab2:
    ...
    for item in stock_display:
        ...
        with c_i:
            st.markdown(f"**{item['종목명']}** ({item['티커']}) | **{item['수익률']:.1f}%**")
            st.caption(f"평가액: {item['평가']:,.0f}원 | 현재가: {item['현재가']:,.2f}{'$' if item['해외'] else '₩'}")
        with c_e:
        ...
        if st.session_state.get(f"em_{item['ID']}", False):
            new_p = st.number_input("평단가", value=float(st.session_state['stocks'][item['ID']]['매수평단가']), key=f"np_{item['ID']}")
            new_q = st.number_input("수량", value=float(st.session_state['stocks'][item['ID']]['보유수량']), key=f"nq_{item['ID']}")
...
with tab3:
    ...
    if abs(t1+t2+t3+t4+t5-100) < 0.2:
        ...
        for sv in st.session_state['savings']:
            grp = "5. 안전(고정)" if sv['종류'] in ["적금", "주택청약"] else "4. 안전(유동)"
            re_items.append({"자산군": grp, "종목명": f"[{sv['종류']}] {sv['상품명']}", "현재금액": int(sv['월납입액']*sv['현재회차']), "현재가": 0, "티커": "SAV"})
        ...
        if not rdf.empty:
            ...
            for idx, row in rdf.iterrows():
                grp = row['자산군']
                if grp == "5. 안전(고정)": default_targets.append(round(row['현재금액']/grand_total*100, 1))
                else:
